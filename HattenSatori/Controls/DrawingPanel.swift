...
class DrawingPanel: DashBorderView {
    ...
    func drawing(start: Bool) {
        if let scrollView: UIScrollView = self.getSuperView() {
            ...
            self.viewBorder.strokeColor = start ? HattenColor.gray800.color.cgColor : HattenColor.gray500.color.cgColor
    ...
    func drawLine(from: CGPoint, to: CGPoint) {
        if let view = tempImageView {
            ...
            if let context = UIGraphicsGetCurrentContext() {
                ...
                context.setStrokeColor(HattenColor.black.color.cgColor)
